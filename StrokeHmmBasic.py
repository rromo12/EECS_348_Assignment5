...
class HMM:
    ...
    def label( self, data ):
        ...
        for s in self.states:
            emmision= 1
            for f in self.featureNames:
                emmision= emmision *  self.emissions[s][f][data[0][f]]
            V[0][s] = self.priors[s] * emmision
            path[s] = [s]
        # Run Viterbi for t > 0
        for t in range(1, len(data)):
            ...
            for s in self.states:
                emmission = 1 
                for f in self.featureNames:
                        emmission= emmission *  self.emissions[s][f][data[t][f]]
                prob,mostLikelyState=max((V[t-1][st]*self.transitions[st][s]*emmission,st) for st in self.states)
                V[t][s] = prob
                newpath[s] = path[mostLikelyState] + [s]
...
class StrokeLabeler:
    ...
    def testHMMDir(self,trainingDir):
        ...
        for files in lFileList:
            ...
            rdict['text']['text'] += confmat['text']['text']
...
# Examples
# ###Website Model
# hmm=HMM(['sunny','cloudy','rainy'],['evidence'],{'evidence':1},{'evidence':3})

# hmm.priors = {'sunny':0.63,'cloudy':0.17,'rainy':0.20}

# hmm.emissions = {'sunny':{'evidence':[0.6,0.20,0.15,0.05]},
#                  'cloudy':{'evidence':[0.25,0.25,0.25,0.25]},
#                  'rainy':{'evidence':[0.05,0.10,0.35,0.50]}}   #evidence model

# hmm.transitions = {'sunny':{'sunny':0.5,'cloudy':0.250,'rainy':0.250},
#                    'cloudy':{'sunny':0.375,'cloudy':0.125,'rainy':0.375},
#                    'rainy':{'sunny':0.125,'cloudy':0.675,'rainy':0.375}} #transition model


# data=[{'evidence':0},{'evidence':2},{'evidence':3}]
...
# hmm=HMM(['sunny','cloudy','rainy'],['evidence'],{'evidence':1},{'evidence':3})
# hmm.priors = {'sunny':0.63,'cloudy':0.17,'rainy':0.20}
# # evidence: dry, dryish, damp, soggy
# hmm.emissions = {'sunny':{'evidence':[0.6,0.20,0.15,0.05]},
#                  'cloudy':{'evidence':[0.25,0.25,0.25,0.25]},
#                  'rainy':{'evidence':[0.05,0.10,0.35,0.50]}}   #evidence model
